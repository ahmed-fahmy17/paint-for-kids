...
void ActionAddSquare::Execute() 
{
	...
	if (topLeft.y <= UI.ToolBarHeight || topLeft.y >= UI.height - UI.StatusBarHeight || topLeft.x + SideLength > UI.width || topLeft.y + SideLength > UI.height - UI.StatusBarHeight) {
		pGUI->PrintMessage("SQUARE IS OUTSIDE DRAWING AREA");
		return;
	}

		
	//Step 3 - Create a Square with the parameters read from the user
	CSquare *R=new CSquare(topLeft, SideLength, SqrGfxInfo, pManager->getFigCount());
